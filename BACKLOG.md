...
> The fetch skip list that never converges: [plan](docs/plans/sync-skip-list.md), five tickets. [#0112](docs/tickets/0112-gate-message-id-rebind.md) is the one that stops the pain and has shipped. The other four are independent of it and of each other and are not cut yet: #0113 bounds pass 2 and lets a slow mailbox yield (perf, now), #0114 drains the mutation queue at the tail of the tick as well as the head (bug, now), #0115 makes a non-converging fetch visible (feature, next, and depends on #0112 so the detector is not written against the bug it would have caught), #0116 stops APPENDing a Sent copy the server files itself (bug, next, the source of the duplicates #0112 now shows honestly).
