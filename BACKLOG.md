...
> Eight follow-ups carry out of it, none of them blocking Phase 3b.
...
> `mailbox.list` opens the account's store twice and lists every row to count the unread ones, where one open and one grouped count query would do; the wire shape does not change with the fix.
...
> `EventQueue::try_recv`, `len` and `is_empty` are called only by the contract tests, and `Change::kind()` maps three kinds (`mailbox.counts_changed`, `draft.removed`, `outbox.counts_changed`) that no longer travel since `Event::from_change` supersedes them; both are for the first phase that touches those files, not a drive-by deletion.
...
> Eight clippy warnings sit in T-unit test files (`daemon_events` 5, `daemon_framing` 1, `daemon_read_only_methods` 1, `cli_help_snapshot` 1) and are left there because an implementer may not edit a pinned test file outside an approved contract fix.
