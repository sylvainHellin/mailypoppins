...
> Four follow-ups carry out of it, none of them blocking Phase 3a.
> `message.list` carries no display date, so `mp --daemon list-messages` reads the `Date:` header from the same store the daemon read; adding `date_display` to the wire shape is a protocol change with a changelog entry and closes the last local read on the routed path.
> The `account.list` and `message.list` fixtures are checked for canonical form and round-tripping but not against a live daemon's answer, where `initialize` is; the missing test is the read-only twin of `initialize_request_matches_the_pinned_fixture_shape`.
