...
> The fetch skip list that never converges: [plan](docs/plans/sync-skip-list.md), five tickets. [#0112](docs/tickets/0112-gate-message-id-rebind.md) is the one that stops the pain and has shipped, and so has [#0116](docs/tickets/0116-one-outbox-drain-per-account.md), whose premise in the plan was wrong: the duplicate Sent copies were mp appending the same row from several concurrent drains, not Exchange filing its own copy. The other three are independent of each other and are not cut yet: #0113 bounds pass 2 and lets a slow mailbox yield (perf, now), #0114 drains the mutation queue at the tail of the tick as well as the head (bug, now), #0115 makes a non-converging fetch visible (feature, next, and depends on #0112 so the detector is not written against the bug it would have caught).


## Next

- [#0117 The rebind gate runs undegraded on the passes that finish a windowed UIDVALIDITY reset](docs/tickets/0117-rebind-gate-after-a-windowed-reset.md) -- bug _(found while pinning #0112's degradation guard; witness test in `src/sync/engine.rs`)_
