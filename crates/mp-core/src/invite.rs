//! iCalendar (iMIP) building and replying (ticket #0028).
//!
//! Builds a `METHOD:REQUEST` `VCALENDAR`/`VEVENT` from resolved input and
...
// ---------------------------------------------------------------------------
// The `mp send --invite` request, shared by the client and the daemon (P4-U12)
// ---------------------------------------------------------------------------

/// `ANO-4`: Graph accounts cannot send iMIP invitations yet (Graph send is
/// #0036, blocked on tenant admin approval #0035).
///
#[cfg(test)]
mod tests {
    use super::*;
