...
pub(super) fn render_body(app: &mut App, frame: &mut Frame, area: Rect) {
    ...
    if !app.preview_lines.holds(epoch, inner_width) {
        ...
        let body = mp_core::draft::strip_signature_sentinels(app.preview_body.text())
...
pub fn event_card_lines(event: &EventFrontmatter, is_sent: bool) -> Vec<Line<'static>> {
...
mod event_card_tests {
    use super::*;
    use mp_core::types::{EventAttendee, EventFrontmatter};
