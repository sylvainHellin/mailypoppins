...
fn draft_attachment_files(app: &mut App) -> Option<Vec<PathBuf>> {
    ...
    let draft = match mp_core::draft::parse_email_draft(&path) {
        Ok(draft) => draft,
        Err(e) => {
...
fn attach_file_to_draft(app: &mut App, path: &str) {
    ...
    match mp_core::draft::append_draft_attachment(&draft_path, path) {
        Ok(()) => {
...
pub fn fetched_attachment_files(
    app: &mut App,
    fetched: &mp_core::parse::FetchedEmail,
    index: usize,
) -> Option<Vec<PathBuf>> {
    let dest = match mp_core::parse::materialisation_dir(&format!("search-{index}")) {
        Ok(dir) => dir,
        Err(e) => {
            app.set_status_level(format!("Attachments failed: {e:#}"), StatusLevel::Error);
            return None;
...
fn write_fetched_attachments(
    ...
    for att in &fetched.attachments {
        let out = dest.join(mp_core::parse::sanitize_attachment_filename(&att.filename));
...
pub fn validate_then_approve(path: &Path) -> Result<mp_core::types::EmailDraft> {
    let draft = mp_core::draft::parse_email_draft(path)?;
    mp_core::draft::validate_draft(&draft)?;
    mp_core::draft::mark_as_approved(path)?;
    mp_core::draft::parse_email_draft(path)
}
...
pub(super) fn handle_action(
    ...
    match action {
        ...
        Action::OpenLogFile => match mp_core::config::latest_log_file() {
            Some(path) => {
                suspend_terminal(terminal)?;
                let result = edit_file(&path);
                resume_terminal(terminal)?;
            ...
            None => app.set_status_level(
                format!(
                    ...
                    mp_core::config::logs_dir().display()
                ),
                StatusLevel::Warning,
            ),
        },
        ...
        Action::OpenConfigFile => {
            let path = mp_core::config::config_path();
            if path.exists() {
                suspend_terminal(terminal)?;
                let result = edit_file(&path);
                resume_terminal(terminal)?;
        ...
        Action::OpenAttachment(path) => match mp_core::parse::open_file_with_system(&path) {
            Ok(()) => {
                let name = path
                    .file_name()
        ...
        Action::SaveAttachments { sources, dest_dir } => {
            ...
            for source in &sources {
                match mp_core::parse::save_attachment(source, &dest_dir) {
                    Ok(_) => saved += 1,
                    Err(e) => {
        ...
        Action::OpenHtmlInBrowser(path) => match mp_core::parse::open_file_with_system(&path) {
            Ok(()) => {
        ...
        Action::ServerSearch { query, targets, local_mailbox } => {
            ...
            let params = json!({
                "account": account,
                "query": mp_core::search::to_query_string(&query),
                "mailboxes": mailboxes,
...
fn open_compose_wizard(app: &mut App, mode: ComposeMode) {
    ...
    let contacts = {
        let root = mp_core::config::account_dir(&app.account_config.name);
        mp_core::contacts::load_cache(&root).ok().flatten()
    };

    // The signature names for the Signature field selector (#0106), read from
    // the app-managed signatures directory (#0107).
    let available_signatures = mp_core::signatures::list();
    ...
    let default_signature = if app.global_config.email.include_signature {
        mp_core::signatures::default_signature_name(&app.account_config.name)
    } else {
        None
    };
...
fn send_contact_as_vcard(
    ...
    let stem = mp_core::contacts::vcard_file_stem(contact);
    ...
    let vcard = mp_core::contacts::contact_to_vcard(contact);
...
/// address local-part.
fn vcard_display_name(contact: &mp_core::contacts::Contact) -> String {
...
fn edit_compose_signature(
    ...
    if !mp_core::signatures::exists(&name) {
        if let Err(e) = mp_core::signatures::write(&name, "") {
            app.set_status_level(format!("Cannot open signature: {e:#}"), StatusLevel::Error);
            return Ok(());
        }
    }
    let path = mp_core::signatures::signature_file(&name);
...
fn edit_signature_file(
    ...
    if let Err(e) = mp_core::signatures::validate_name(name) {
        app.set_status_level(format!("Cannot open signature: {e:#}"), StatusLevel::Error);
        return Ok(());
    }
...
fn submit_compose_wizard(
    ...
    if let ComposeMode::EditDraft { id } = &wizard.mode {
        ...
        match mp_core::draft::rewrite_draft_recipients(&path, &edit) {
            Ok(()) => {
                ...
                if sig_changed {
                    ...
                    if let Err(e) = mp_core::draft::rewrite_draft_signature(
                        &path,
                        sig_md.as_deref(),
                        sig_name.as_deref(),
                    ) {
...
fn wizard_signature(app: &App, wizard: &ComposeWizard) -> (Option<String>, Option<String>) {
    ...
    let md = mp_core::config::resolve_signature_markdown(&app.account_config, name.as_deref());
    (md, name)
}
...
fn slugify_subject_for_filename(subject: &str) -> String {
    let slug: String = subject
        ...
        .collect();
    let slug = mp_core::types::collapse_hyphens(&slug);
...
fn handle_search_result_action(
    ...
    match action {
        ...
        Action::SearchResultOpenInBrowser => {
            ...
            match mp_core::parse::open_file_with_system(&path) {
...
mod tests {
    ...
    fn a_signature_file_edit_respices_an_edit_draft_on_submit() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = mp_core::config::test_env::ConfigDirOverride::new(dir.path());
        cfg.set_config_dir(&dir.path().join("config"));
        let _data = mp_core::config::test_env::TestDataDir::new();
        mp_core::signatures::write("work", "-- \nAlice").unwrap();
        ...
        mp_core::draft::rewrite_draft_signature(&path, sig_md.as_deref(), sig_name.as_deref())
            .unwrap();
