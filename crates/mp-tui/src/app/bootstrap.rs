...
impl App {
    ...
    /// `global_config` supplies each account's [`mp_core::config::AccountConfig`]
    ...
    pub fn from_bootstrap(
        ...
        // the process.
        let theme_warning = crate::theme::init(&global_config.theme);
        ...
        let accounts: Vec<AccountState> = bootstrap
            ...
            .map(|account| {
                let config = global_config
                    ...
                    .cloned()
                    .unwrap_or_else(|| mp_core::config::AccountConfig {
    ...
    fn land_bootstrap(&mut self, bootstrap: &Bootstrap, resync: bool) {
        ...
        for diagnostic in &bootstrap.snapshot.diagnostics {
            crate::events::land_check(self, diagnostic);
        }
...
mod tests {
    ...
    fn config(name: &str) -> mp_core::config::GlobalConfig {
