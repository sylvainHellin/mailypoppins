...
pub struct App {
    ...
    /// inside [`crate::ui::preview::render_body`], the one place that
    /// knows the pane's width.
    pub(crate) preview_lines: crate::ui::preview::PreviewLinesCache,
    ...
    /// The undo-send hold the daemon is counting down, as its last
    /// `send.hold_*` event described it (`SND-04`, P6-U2).
    ///
    /// What this client renders and nothing more: the timer, the parked send
    ...
    pub global_config: mp_core::config::GlobalConfig,
...
thread_local! {
    /// How many preview-query spans this thread has opened, for the test that
    /// pins "one span per cursor move".
    ///
    /// Thread-local because the store fixtures are
    /// ([`mp_core::config::test_env::TestDataDir`], #0077): a process-wide
    ...
    pub static PREVIEW_QUERY_SPANS: std::cell::Cell<usize> = const { std::cell::Cell::new(0) };
}
...
fn preview_query_span(context: String) -> mp_core::timing::TimingSpan {
    #[cfg(any(test, feature = "test-support"))]
    PREVIEW_QUERY_SPANS.with(|n| n.set(n.get() + 1));
    mp_core::timing::TimingSpan::with_context("tui_preview_query", context)
}

impl App {
    pub fn new() -> Self {
        let global_config = mp_core::config::load_global_config().unwrap_or_default();
    ...
    /// active.
    pub(super) fn shell(
        global_config: mp_core::config::GlobalConfig,
        ...
        Self {
            ...
            drafts_dir: None,
            account_config: mp_core::config::AccountConfig::default(),
    ...
    pub fn default_for_tests() -> Self {
        Self {
            ...
            server_search_generation: 0,
            server_search_operation: None,
            global_config: mp_core::config::GlobalConfig::default(),
            session: None,
            events: super::events::EventState::default(),
        }
    }
    ...
    pub fn ensure_contacts_loaded(&mut self) {
        ...
        let root = mp_core::config::account_dir(&self.account_config.name);
        self.contacts_view.index = mp_core::contacts::load_cache(&root).ok().flatten();
    ...
    pub fn recompute_contact_matches(&mut self) {
        ...
        let matches: Vec<String> = match &self.contacts_view.index {
            Some(index) => mp_core::contacts::search(index, &query, usize::MAX)
                .into_iter()
    ...
    pub fn recompute_calendar_visible(&mut self) {
        let now = mp_core::calendar::now_sort_key();
    ...
    pub fn drafts_dir(&self) -> PathBuf {
        self.drafts_dir
            .clone()
            .unwrap_or_else(|| mp_core::config::drafts_dir(self.account_name()))
    }
    ...
    pub fn update(&mut self, msg: Message) -> Option<Message> {
        match msg {
            ...
            Message::Quit => {
                // Quitting mid-hold was refused until P6-U2, because the hold
                // lived in this process and closing the window would have
                // dropped a send the user confirmed. The daemon owns the timer
                // now and has its own rule for a window that goes away -- the
                // send survives while another client watches, and the *last*
                // client leaving cancels it and leaves the draft approved -- so
                // a refusal here would only make that rule unreachable.
                self.running = false;
                None
            }
        }
    ...
    /// same way.
    pub fn refresh_preview_body(&mut self) {
    ...
    pub fn refresh_preview_invite(&mut self) {
        let key = self.preview_body_key();
    ...
    pub fn load_message_ics(&self, msg: MessageRef) -> Option<Vec<u8>> {
        let account = self.account_config.name.clone();
        let queries = self.queries()?;
    ...
    pub fn draft_body(&self, id: &str) -> Option<String> {
        let account = self.account_config.name.clone();
        let queries = self.queries()?;
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn prime_preview_body(&mut self, text: impl Into<String>) {
        let key = self.preview_body_key();
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn prime_preview_invite(&mut self, event: mp_core::types::EventFrontmatter) {
        let key = self.preview_body_key();
    ...
    pub fn refresh_signature_content(&mut self) {
        ...
        for acct in &mut self.accounts {
            acct.signature_content = if include {
                mp_core::config::resolve_signature_markdown(&acct.account_config, None)
            } else {
                None
            };
        }
        self.signature_content = if include {
            mp_core::config::resolve_signature_markdown(&self.account_config, None)
        } else {
            None
        };
