...
- **Two-pass fetch.** Pass 1 fetches `BODY.PEEK[HEADER.FIELDS (Message-ID)]` only (~50 bytes / msg). Pass 2 fetches full `RFC822` for unknown Message-IDs. Both passes share one IMAP session.
