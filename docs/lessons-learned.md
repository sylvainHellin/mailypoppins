...
Lettre's `Mailbox::from_str` is a strict RFC 5322 parser: a display name must be either an `atom` (atext only) or a `quoted-string`. Real senders routinely violate this -- TUM mailing lists like `CCBE_Researchers [TUBVCMS] <researchers.ccbe@ed.tum.de>` ship unquoted `[`/`]`, and unquoted commas in `Last, First <addr>` are also common. Both `validate_draft` and `send_email` previously parsed the raw frontmatter string directly, so any such header produced `Send failed: Invalid email address: ...` and the user could not send. Fix: `send::normalize_address_for_smtp` auto-quotes display names whose characters fall outside RFC 5322 atext + FWS + `.`, escaping inner `\` and `"` per the quoted-string rules. Both validation and the actual `to`/`cc`/`bcc`/`from`/`reply_to` parses now route through it. Bare addresses, atext-only names, and already-quoted names pass through unchanged. Tests live in `src/send.rs` under `normalize_*`.
