...
`"Hellin, Sylvain" <addr>` historically broke send/validate because `.split(',')` split inside the quoted name. Use `split_addresses()` from `parse.rs` for any address-list parsing. Don't reach for `.split(',')` on email headers.
