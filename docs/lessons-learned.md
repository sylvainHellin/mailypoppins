...
## The HTML rendition's CSP names what it allows, and `<meta http-equiv=refresh>` is outside CSP

The browser rendition of a message gets a `<meta http-equiv="Content-Security-Policy">` tag (`inject_csp_meta` in `crates/mp-core/src/parse.rs`) so that opening it from disk cannot run scripts or load remote tracking pixels. The policy is `default-src 'none'; img-src data:; style-src 'unsafe-inline'; font-src data:; form-action 'none'; base-uri 'none'`: `embed_inline_images` rewrites every `cid:` reference to a `data:` URI before the file is written, so no `file:` source is needed, and `form-action` and `base-uri` do not fall back to `default-src`, so they have to be named. A `<meta http-equiv="refresh">` would navigate the tab to a remote URL whatever the CSP says, so `strip_meta_refresh` removes every such tag first, looping until none reassembles from a nested one. The rendition is then handed to `open` on macOS and `xdg-open` elsewhere, spawned with null stdio and never waited on, because the TUI calls it from its draw loop and anything the opener prints would land on a raw-mode screen.
...
The rank is therefore `(sequence, dtstamp, is_organizer, mailbox, uid)`, with the organizer flag load-bearing above the identity, in [src/agenda.rs](../src/agenda.rs) (#0034, carried onto rows in #0038, moved out of `src/tui/` in #0126).
...
## The `/tmp` tmpfs is too small for a full parallel run, and the failure does not look like a resource failure

Most of the ~2200 tests build a sqlite store inside a `tempfile::tempdir()`, and on this box `/tmp` is a RAM-backed tmpfs the kernel caps per user at 80% of its size. A full parallel run exhausts that ceiling and the tests report `Disk quota exceeded` and `SQLITE_IOERR`, which reads as a store bug rather than as the suite running out of room. The workaround, `TMPDIR=/var/tmp cargo test --workspace`, was recorded in handoff notes from 2026-08-07 onward and in ticket 0124, but only as a sentence in front of a command, so any session that did not read those notes ran without it.

`[env] TMPDIR = "/var/tmp"` in `.cargo/config.toml` removes the discipline: cargo applies that table to the processes it spawns, test binaries included, so `std::env::temp_dir()` resolves to disk with nothing at the call site. `force` is deliberately off, so an explicit `TMPDIR` from the caller still wins. Agent scratch is the same lesson learned the hard way from the other side: the 2026-09-09/10 stub-proof sessions left 9 GB of worktrees in `/tmp` and 17 GB of `CARGO_TARGET_DIR` trees in `~/.cache`, and the `/tmp` half broke unrelated tooling days later. The general rule: an environment precondition a run depends on belongs in the repo's own config, not in a sentence somebody has to remember to type.

## A log two processes append to can tear a line, so a parser has to anchor on the timestamp

The first-paint harness of P5-U11 read the TUI's own `[TIMING] tui_draw` line out of `<data_dir>/logs/mailypoppins-YYYY-MM-DD.log` and subtracted the exec instant. About one run in six came back at 250 to 290 ms where its neighbours were at 7 or 35, which looks exactly like a real tail and is not one: the client and the daemon both write to that file through `simplelog`, which emits a record in more than one `write`, so a concurrent record can land in the middle of one and leave a line beginning `[INFO] [TIMING] …` with no timestamp on it. A naive `grep -m1 'tui_draw done'` then matched the torn line, failed to parse, or skipped to the *next* frame, which is one 250 ms idle tick later.

Matching only `^[0-9]{4}-[0-9]{2}-[0-9]{2} .*tui_draw` removed the whole family of outliers, and the medians went from a bimodal 35/85 to a 34-36 band. The lesson generalises past this harness: anything that measures mp by reading its log has to anchor on the timestamp at the start of the line, and an outlier distribution that is bimodal rather than long-tailed is usually an artifact of the instrument rather than a property of the thing.

## A manual key checklist needs a fixture arranged for it, or it proves nothing

Oracle (d) of the Phase 5 gate is twenty hand-dispatched keys pressed against a real TUI. Three of them scroll, and a fresh session's activity log holds three entries, so `j`, `d`, `gg` and `G` all leave the screen exactly as they found it and a checklist row saying `pass` would be a lie with evidence attached. The same trap sits under `d`/`u` in the search result list, where every fixture body is shorter than the preview pane, and under `o`/`O`, where the hits have to carry attachments.

What made the walk honest was arranging the state first: the activity log filled to its 100-entry cap with a ladder of distinguishable lines (`gt` with a different token each time, so every press leaves `Cannot read 'nNN' as a date`), the search form's attachment toggle turned on so every hit carries a part, and a hit whose body is long enough for a half-page scroll to move text. Then each key's effect is read off the screen rather than assumed. Budget that arranging as part of the walk: it was more work than pressing the keys.

## A ledger's line count is a property of the fixture, not of the thing under test

Three rows of `tests/daemon_send_hold.rs` assert `transport_events(&log).len() == 1` after a successful send of `send_fixture::APPROVED`. Nothing an implementation can do makes that true: `widen_approved_draft` puts a second address on the draft's `cc:`, the fake transport writes one `Submit` line **per recipient**, the send files its own Sent copy, and the send's outbox drain finishes the seeded `APPENDING_ROW`'s APPEND on the way out. One send is four lines, and the zero-window row proves it is not about the hold, because it takes the ordinary unheld path and produces the same four.

`tests/daemon_send_slice.rs` had already learned this and says so where it counts by `Message-ID` instead: *"SND-09 is about this message's copy, counted by its Message-ID rather than by the ledger's length"*. The general shape is worth naming, because a contract test written against a shared fixture is the easy place to make it: an assertion on **how many** things a shared ledger holds is an assertion about every other thing the fixture seeds, so count the ones you can name (a message id, an address) and never the file's length.
...
## simplelog writes the target and the thread only at `DEBUG` and below

A line of `<data_dir>/logs/mailypoppins-<date>.log` is not one format. `WriteLogger` writes a field when `config.<field> <= record.level()`, and the default `Config` sets `time` and `level` to `Error` (so both are always written, `Error` being the smallest `LevelFilter` above `Off`) but `thread` and `target` to `Debug`. An `INFO` line is therefore `2026-09-21 19:25:59.921 [INFO] message` with no target at all, while a `DEBUG` line is `… [DEBUG] (3) mailypoppins::daemon: message`.

A parser that looks for a `target: ` prefix at every level eats half of any message with a colon in it - `warning: the disk is full` becomes target `warning` - and a parser that requires one drops every `INFO` line. `diagnostic.logs` reads the thread and the target at `DEBUG` and `TRACE` only, which is what the writer emits, and that is a property of the logger's configuration rather than of the file, so it moves if `init_logging` ever changes `set_target_level`.

## A daemon's descriptor count is a high-water mark, and its first reading is not it

The first soak row compared `/proc/<pid>/fd` before and after 200 connect/bootstrap/disconnect cycles and read +8 descriptors, which looks exactly like a leak of one socket per few clients. It is not: a daemon that has just bound its socket is still starting, and what the churn does is finish the startup. The account runtimes come up off the startup path, the read pool opens its two connections on the first read that needs them, and SQLite keeps a further handle per concurrent reader for as long as the process lives. Measured here: 22 descriptors cold, 37 after the first few dozen concurrent clients, and 37 after four thousand more.

So a before/after descriptor assertion has to warm the daemon up first, wait for the count to stop moving, and only then take the baseline - and the "after" reading has to wait for stability too, because a client's `close` returns before the daemon's read sees the end-of-file and the connection task that owns the descriptor is reaped after that. Asserting "back within a small margin of a warm, settled reading" is a real assertion; asserting it against a cold one is a coin toss the daemon loses.

## A registry that keeps history keeps it for the life of the process

`OperationRegistry` held every operation the daemon had ever started so `operation.status` could still answer for one that had finished, which reads as a convenience and measures as a leak: about 0.8 KiB per settled operation, and a daemon that quick-syncs every five minutes accumulates a hundred thousand of them in a year. Nothing in the functional suite could see it - every test starts a fresh daemon and runs a handful of operations - and no absolute memory ceiling would have caught it either, because it only becomes visible at a scale a ceiling generous enough not to be flaky has already allowed.

What caught it was reading the *same* row's growth at two scales: 200 passes grew the resident set by 0.6 MiB and 3600 grew it by 2.7. That is the shape to look for in a soak, and it is why every row of `tests/daemon_soak.rs` prints its numbers instead of only asserting a bound. The cap that fixed it also made the daemon markedly faster under load, because `live()` and every disconnect walked the whole table: 113 holds a second became 767.

## A rerun of a benchmark re-takes the baseline column, or it measures the filesystem

The Phase 6 benchmark rerun put its fixture on `/var/tmp` (ext4) where the Phase 4 table had used `/tmp` (tmpfs), which is the right move for a daemon-era measurement and the wrong one to compare against a stored number. Every oracle figure came back 4 to 9 ms above the figure the same binary produced in Phase 4, and a rerun that had quoted the old baseline instead of re-taking it would have reported a uniform regression that is nothing but the disk.

The rule the pre-daemon `workloads.md` already states and this is the demonstration of: a before/after pair is one session, one filesystem, one machine state, and every column of it is taken again. A stored number is a record of what a *past* session measured, not a baseline a later session may subtract from. Where the columns genuinely cannot be re-taken together, the older one goes in the table as a direction rather than as a delta, and the text says which filesystem each was on.

## A wire row that grows is invisible to every test and visible only to a benchmark

`message.list`'s `list` projection carried nine keys per row in Phase 4 and fifteen after Phase 5, when the TUI started reading its list rows over the socket and needed `id`, `to`, `cc`, `reply_to`, `bcc`, `is_invite` and the `flagged` bit. Every test stayed green, because each one asserts on the fields it reads and nothing asserts on the width of a row. The cost is per row and therefore invisible at test sizes: it showed up only in the 5000-row workload, where the per-row cost went from about 9 µs to about 17 and the whole answer from 56 ms to 94.

Two things follow. A projection named `list` is a contract about size as well as about shape, and a field added to it for one caller is paid for by every caller at every row. And a benchmark table is worth re-running per phase for exactly this class of change, which no boundary test, count tripwire or byte-parity oracle can see, since the output the oracle compares is the rendered listing and not the frame that carried it.

## A two-mutex registry needs a written lock order, and one method is enough to break it

`HoldScheduler` keeps the holds in an `armed` map and their arm order in an `order` vector, each under its own `Mutex`. Three of its four methods that need both take `armed` first and `order` inside it; `cancel_all` took `order` for the whole of a `collect()` and reached for `armed` inside the filter closure. That is a deadlock one interleaving away, and both interleavings are reachable from a live socket: `cancel_all` runs from step 2 of a shutdown and from the last client disconnecting, `listing` from `send.hold_status`, `diagnostic.health` and `state.bootstrap` on another connection. Nothing in the functional suite could see it, because every row drives one connection at a time.

Two rules came out of it. Write the order on the struct, because a reader checking a new method against three existing ones will not notice that the fourth disagrees, and a method that needs only the second lock takes it alone and drops the guard before it touches the first: `let doomed: Vec<_> = lock(&self.order).clone();` is the whole fix, and the id that left the map between the snapshot and the cancel is what the cancel's `false` return already meant. Pin it with a row that runs the two methods against each other on two threads and reports through an `mpsc` channel, so the failure is a `recv_timeout` rather than a test binary that hangs until the harness is killed.

## `#[cfg(test)]` does not cross a crate boundary, and a test seam that moves out silently stops firing

Moving eleven modules from the root package into `crates/mp-core` (P5-U10a) is a `git mv` plus a re-export for every one of them but two. `config::test_env` holds the thread-local `$HOME` / `$MAILYPOPPINS_DATA_DIR` overrides that #0077 introduced so that parallel tests stop racing on `environ`, and `config`'s three path resolvers read it behind `#[cfg(test)]`; `parse::materialisation_root` returns a per-thread root behind the same attribute. A dependency is compiled without `cfg(test)` whatever the crate depending on it is doing, so both seams would have evaluated to their production branch the moment the files stopped belonging to the root crate: every `TestDataDir` in the tree would have pointed at the real `$HOME`, and nothing would have failed to compile.
