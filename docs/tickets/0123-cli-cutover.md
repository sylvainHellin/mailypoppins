...
- **(d) The two wizards**, six rows. What the wizards ask the daemon for is `config.get`, through `routed_config_state` (`src/main.rs`): the path, whether the file exists, and the account names. `config.init` and `config.add_account` are served and are what a wizard protocol would build on, but no wizard calls either; only `tests/daemon_config.rs` exercises them, which is now a line in `BACKLOG.md`. What has no wire shape is the prompting loop, whose intermediate results steer the next prompt, and one of the six rows is `set_secret(`: the wizards store the password they prompted for themselves (`src/config_cmd/init.rs:227,286,633,673`) rather than through `config.set_password`.
