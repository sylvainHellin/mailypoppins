...
`diagnostics` carries the daemon's health checks that are **not** `ok`, in the order `diagnostic.health` reports them, each entry being that report's `checks` item verbatim: `{name, status, detail}`.
A healthy daemon answers an empty array.
It is the checks that are not `ok` rather than all of them because a client renders this as a complaint list, and a daemon that handed over six passing checks would make every window open with six lines saying nothing happened.
...
`diagnostic.check_changed` says that one of the daemon's health checks moved, with a payload of `{name, status, detail}`, the same three keys `snapshot.diagnostics` and `diagnostic.health` carry.
It is a lifecycle event and published on a change of `status` only, never for a detail that moved under an unchanged status: a daemon that republished its whole check set on every evaluation would fill a client's activity log with news that nothing happened.
A name a client has not seen before was `ok` as far as the daemon is concerned, so the first evaluation after a startup announces what is wrong and says nothing about what is right.
...
A row is `mp_protocol::calendar::AgendaEvent` `{row_id, event, subject, start_sort, end_sort, start_display, is_organizer, cancelled}`, and no key is skipped when empty.
`message.invite` `{account, id|row_id|selector, mailbox?}` -> `{account, row_id, event}` answers one message's invitation card, the same fold over one payload; `message.ics` with the same address -> `{account, row_id, ics}` answers the row's raw `invite.ics` blob, base64 in the standard alphabet with padding, because a blob is bytes and a JSON string is not.
Both answer `null` for a row that carries no iMIP payload rather than refusing: a non-invite is the ordinary case, and the preview shows no card for one exactly as it always did.
`calendar.events` is declared in `CALENDAR_QUERY_METHOD_SPECS` and the two `message.*` reads in `MESSAGE_INVITE_METHOD_SPECS`, arrays of their own for the reason `MESSAGE_QUEUE_METHOD_SPECS` is one: the existing arrays are pinned by tests written about the slices that created them, and the split is a Rust-side fact the wire never sees.
The capability list a handshake advertises grew by those three names, which is the derivation working rather than a change to it.
`mp_protocol::calendar` also now owns `EventFrontmatter` and `EventAttendee`, which were `mailypoppins::types`': pure serde structs both ends of the socket need, re-exported from the root crate under their old paths, so nothing about the YAML frontmatter they also serialise moved.
...
`version` and `protocol_version` are deliberately not `app_version` and `protocol`: `daemon.status` is the pre-handshake lifecycle probe and answers the daemon's *range*, while health is a support document and what a support case needs is the single integer this connection negotiated.
An `accounts` entry is `{name, runtime, watcher, last_sync: {finished_at, outcome}}`, `runtime` being `daemon.status`'s `opening`/`ready`/`blocked` and `watcher` being `running` or `stopped`; `last_sync` is `{null, null}` until a tick lands.
`store` is one entry and not one per account, because the only question a health report is asked about the store is whether the cache is large.
`checks` is four fixed checks in this order - `config_loaded`, `store_open`, `socket_owner`, `log_writable` - then one `account:<name>` per configured account in configuration order, each `{name, status, detail}` with a `status` of `ok`, `warn` or `fail` and a `detail` that is never empty.
An account blocked because another engine holds its lock is `warn` and one whose runtime never came up at all is `fail`: a second `mp` holding a lock is a normal state of a developer's machine, and a daemon that called it a failure would cry wolf.
`diagnostic.log_path` `{}` -> `{path}` names the file the daemon is writing, `<data_dir>/logs/mailypoppins-<date>.log`, which is the surface `docs/parity-matrix.md` `INT-02` and `OBS-05` reach the log through.
`diagnostic.logs` `{lines?, level?, since?}` -> `{path, lines, truncated}` parses that file: `lines` defaults to 200 and above 5000 is `-32602` naming the cap rather than a silent clamp, `level` is one of `trace|debug|info|warn|error` and is a *minimum*, and `since` is any RFC3339 instant compared on the instant rather than on the string.
A line is `{ts, level, target, message}`, `ts` being RFC3339 **with the offset the daemon's local zone had at that wall time** (a winter line read in summer keeps its winter offset; a stamp in the spring-forward gap takes the current one) because the file's own stamps carry none and a bundle read on another machine would otherwise lie about when something happened; `target` is `""` for a line that carries none, and a line the format does not explain is kept whole as `{null, null, "", <the line>}` and survives a `level` filter.
The answer is the tail, in file order, and `truncated` says whether older matching lines were dropped.
`diagnostic.support_bundle` `{out?, redact?}` is a durable operation settling `{path, files, redactions}`.
It writes a **directory** of five files - `config.toml`, `daemon-status.json`, `health.json`, `log.txt`, `version.txt`, reported in that order - and not an archive, because this tree links neither `tar` nor `flate2` and a user who wants one attachment runs the `tar` his machine already has; adding the archive later is a wrapper rather than a reshuffle.
`out` is absolute or it is `-32602`, the rule every path that crosses the socket follows, and absent it lands under the data directory.
`redact` defaults to `true`: the keys `password`, `client_secret`, `access_token`, `refresh_token` and any `*_secret` or `*_token` decide what is a secret, and every value so identified is struck from **every** file of the bundle with the literal `<redacted>`, `log.txt` included, because a library that logged a credential did not know it was one.
The key stays, so a reader sees that a password was set; email addresses and OAuth2 client ids stay verbatim, since a bundle without them is useless and neither is a credential.
