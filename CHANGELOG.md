...
- **A reply goes to the `Reply-To:` mailboxes, every one of them, and falls back to `From:`.** Reply-all keeps the original sender in `cc:` and never repeats a `Reply-To:` address there.
- **The terminal window is its own crate now, and it cannot reach your mail without asking the daemon (#0126).** The last unit of the move that started five releases ago: everything the window is - the list, the keys, the panes, the overlays - is `crates/mp-tui`, which links the shared configuration and parsing code, the wire types and the socket, and nothing else. It cannot open your store, your keyring or your mail server, and that is now a compile error rather than a rule someone has to keep. Nothing you press changes and nothing you look at moved: the twenty golden frames, `mp --help` and `mp dump-keys --json` are byte-identical, and the same 2 414 tests run. One thing behaves differently in a corner nobody should ever reach: a window whose daemon connection wedged at startup now shows an empty list and an empty preview with a line in the log, where it used to read your store directly behind the daemon's back.
- **The conversation overlay, the reply to a hit only the server has, and the drafts list are the daemon's now (#0126).** Nothing you press changes and nothing you look at moved, which is the point: `tt` still opens the thread a message belongs to, `r`, `R` and `w` on a search hit that is not in your store still quote what is on screen without downloading it, and the Drafts list still catches up when an agent or `$EDITOR` writes a file behind the application's back. What changed is who does the work. The conversation is folded where your store is, because which messages share a thread is a query over every mailbox of the account and the window holds one mailbox at a time. The reply to a server-only hit travels as the message rather than as an address, which is what keeps the reply key from silently downloading the message the way the overlay's separate `f` key does. And the once-a-second scan of the drafts directory is gone: the daemon has watched every account's drafts directory for a while now and says so, so the list reacts to a write in any account rather than only the one you are looking at, and it no longer costs a directory walk four times a second whether anything changed or not. One thing was removed outright: the outbox counts the window read on every account open and every finished sync went nowhere, since the badge that showed them left with the old status bar.
- **The contacts refresh no longer freezes the window, and the agenda moved to where your mail lives (#0126).** Pressing `r` in the contacts view walked every message of every mailbox on the drawing thread: on a real account that is seconds of a window that does not repaint. It is a daemon operation now, so the view stays live while it runs and the same four lines you have always read appear when it finishes, the two refusals that protect a good cache from a thin rebuild included. Nothing else about it changed. Under the surface two more things left the TUI: the agenda the calendar view shows is built by the same code the daemon serves it from, in the crate that owns your store rather than in the client that displays it, and the invitation source `Enter` opens in `$EDITOR` is read over the socket like every other read. The TUI's action layer now reaches your store, your mail server and the send path through daemon methods and through nothing else.
- **The search overlay fills in as the server answers, and the last thing the TUI dialled itself is the daemon's now (#0126).** Pressing `ff` used to show the local hits, then sit on `Searching server...` until every mailbox had answered and paint the lot at once, from an IMAP session the TUI opened on a thread of its own. The search runs in the daemon now and each hit lands as it arrives, so a slow folder no longer holds up the fast one, a message found both locally and on the server still appears once, and a folder the server refuses is reported at the end instead of losing you the other four. `f` on a server-only hit ingests it through the daemon too, and pressing it on a message that is already there says so rather than fetching it twice. Opening a message read-only in `$EDITOR` (`Enter`, `e`) and copying its path or its `mp://` name (`y`) go the same way; the rendition is still the store's own Markdown, still read-only, and the copy is reclaimed when you leave the editor. Nothing you look at moved: the twenty golden frames, `mp --help` and `mp dump-keys --json` are byte-identical. `mp search`'s own server leg is deliberately not routed yet and still opens its own session.
- **The rest of the shared half followed it (#0126).** The second of the three units that put the TUI behind a crate boundary, and like the first it changes nothing you run: `contacts`, `draft` and `reconcile` are split at the line where they stop being about a file and start being about your store, the engine-free halves join `crates/mp-core` beside the rest, and the four RFC 5322 address-string helpers move out of the sending code that happened to own them. Every old path still resolves, the help surface and the key dump are byte-identical, and the test count did not move: 87 tests followed their code into the new crate and the guard that counts them was raised to say so. One thing did change under the surface: the draft preview asks the daemon where the file is and reads it itself, rather than opening your store to look the draft up.
- **The shared half of mp is its own crate now (#0126).** Nothing you run changes and nothing you look at moved: this is the first of the three units that put the TUI behind a crate boundary, and all it does is move the modules the client and the engine both read - configuration, secrets, the OAuth2 token cache, signatures, RFC822 parsing, iCalendar, the search grammar, the `mp://` selector grammar and the shared types - out of the root package into `crates/mp-core`, re-exported under every path they had. The help surface, the key dump and the twenty golden frames are byte-identical, the test count did not drop, and the 329 tests that moved with the code are counted in their new home by the same guard that has watched the workspace layout since Phase 2.
- **The daemon can be left running now: it starts at login, holds the undo-send window itself, stops without dropping work, and can be asked how it is doing (#0125).** Until this release the daemon was something a client started when it needed one. Four things change that. The undo-send window moved out of the TUI: pressing send parks the message for `email.send_hold_secs` in the daemon rather than in whichever window you happened to send from, so the countdown is the same countdown in every window you have open, `u` cancels it from any of them, and quitting the TUI mid-window no longer refuses to quit. It cancels the send and leaves the draft approved, which is what closing the last window has always meant and now says so. `mp send` and `mp send-approved` still send immediately, and `send_hold_secs = 0` still opts out. Stopping is graceful: `mp daemon stop` cancels every armed hold, tells every connected client that the daemon is leaving, waits up to ten seconds (`--grace-secs N` to change it) for work already running to settle, releases the account locks, and names on stdout anything it had to cut short. `SIGTERM` runs the same sequence, so a daemon stopped by the system at logout is as careful as one you stopped by hand, and a TUI whose daemon goes away says `Daemon stopping` and reconnects rather than dying with it. `mp daemon install-service` writes a systemd user unit on Linux or a launchd agent on macOS and enables it, so the daemon is up before you open anything; `--check` reports whether one is installed, `uninstall-service` removes it, and an install over a file you edited yourself is refused rather than overwritten. The macOS half is written and pinned against a committed fixture but has never been run against a real `launchctl`, which is the one part of this release that wants a careful first try. And the daemon answers questions about itself: `mp daemon health` prints one line per account and one per check (the configuration file, the stores, the socket, the log) with a verdict and an exit code a script can branch on, `mp daemon logs` tails the structured log with a level filter and no banner around it, and `mp daemon support-bundle` writes a directory holding the configuration, the health report, the status, the version and the log with every password, client secret and token struck out of all five, `secrets.enc` and the token cache never copied at all. A check whose status moves is announced to every open window, where it lands in the activity log rather than over whatever you were doing. One bug was found by the sustained-load tests this release also adds: the daemon remembered every operation it had ever run, about 0.8 KiB each, forever, so a long-lived daemon grew without bound and got slower as it went. It now keeps the last 256 finished ones and forgets the rest, which on a 60-second mixed load run took it from 113 to 767 holds a second.
- **The TUI now answers from the daemon too, so the whole of mp is one engine behind whatever you have open (#0124).** Until this release the TUI opened your store itself on every cursor move, ran its own IMAP IDLE thread per account, and held the account's engine lock for as long as it was running, which is why `mp sync` in another terminal refused to do anything while it was up. The list, the counts, the preview body, the agenda, the invitation card, every archive, delete, flag and move, both sync keys, send-approved and RSVP all go over the socket now, and the watcher moved into the daemon with them. What you should notice is that the two halves of mp stop being rivals: a message archived in the TUI is at its new address for `mp list-messages` in the next terminal immediately, a sync started from either one is the same pass rather than two fighting for the same mailbox, and the mail that arrives while you are reading reaches the open window as an event instead of as a poll. Nothing you look at moved. The twenty golden frames are byte-identical, `mp --help` and `mp dump-keys --json` are byte-identical to the capture taken before any of this work started, and the twenty overlay keys that no test can press were walked by hand against a daemon-backed build and recorded key by key. Startup is unchanged too: with a daemon already running the first frame is on screen in 7 ms against the old binary's 6, and if there is none the TUI starts one and paints in 35 ms, once per daemon lifetime. When the daemon dies under it, the TUI says `The daemon is not reachable; reconnecting…` on the activity line, starts a replacement and re-reads the whole state from it; what it deliberately does not do is open your store itself to carry on, because two engines on one account is the thing this architecture exists to prevent. Two behaviours changed and are worth knowing. An account whose mail you have never synced now comes up `blocked` with the line telling you to run `mp sync` first, where a half-built local database would otherwise report an empty inbox as though that were the truth. And an attachment or an HTML rendition you open from the TUI is materialised into the daemon's own handle directory, which pins it against the retention sweep while your viewer has it open and reclaims it ten minutes later, rather than into a temp directory that lived until the system cleaned it. Two things stay where they were on purpose: the undo-send hold is still the TUI's own, moving into the daemon next phase, and the server leg of the search overlay is the last part of the TUI that still dials IMAP itself.
- **Every `mp` command now answers from the daemon, and the code that used to answer it in process is gone (#0123).** Until this release the daemon was a cargo feature that shipped disabled, and `cargo install --path .` gave you an `mp` that opened your store, your secrets and your mail server itself. From here there is one build: the feature is gone, a command that touches your mail talks to a background daemon over a Unix socket, and if none is running it starts one and retries, then exits 4 naming `mp daemon run` if that fails too. Nothing you type changes and nothing you read changes: every one of the six migrated slices - reading mail, drafts, archive/delete/open/save, sync and watch, sending and the outbox, and the whole of `mp config`, `mp contacts`, `mp calendar`, `mp invite`, `mp store gc` and `mp cutover` - is compared byte for byte against a binary built from the pre-daemon tag, over seeded fixtures, for every flag and every refusal, and `mp --help` is byte-identical to the baseline captured before any of this started. What you should notice is speed. A one-shot query that read the store in process cost 42 ms and costs 10 ms now, because the daemon is already holding the configuration and the open database and your command pays a round trip instead of an open; a search is 41 ms to 10, a message read 44 to 11, a drafts listing 42 to 9. The exception is a whole-mailbox NDJSON export, which is slower routed (86 ms to 121) because several megabytes now cross a socket, and the first command with no daemon running pays about 65 ms extra once to start one. Five commands deliberately never contact a daemon, so they still work when nothing is listening: `mp daemon *` itself, `mp --help`, `mp --version`, `mp dump-keys` and `mp config path`; `mp config init` is deliberately not among them, since it wants to know where the configuration is before it asks you anything. Three behaviours changed and are worth knowing. `mp watch --mailbox` other than INBOX now watches INBOX and says so on stderr, because the continuous watcher has always been INBOX-only and a per-mailbox one is a second connection class nobody has asked for. `mp contacts search` / `mp contacts stats` against an account with no local store now fail with a reason where they used to print an empty index. And a relative path in a draft's `attachments:` list, `attachments: [report.pdf]` with no leading `~` or `/`, now means the file next to the draft rather than the file in the directory you were standing in when you typed `mp send`: the daemon reads the draft and sends the message, no attachment path is ever sent to it, and the directory a daemon was started from is not one anybody chose. An entry written as `~/…` or as a full path is unaffected, and so is one you attached from the TUI, which stores the path you picked. Under the surface, the direct engine paths every migrated command used were deleted, a thousand lines of them, and a new test walks the client sources for anything that opens a store, a secret backend, a network backend or an engine lock, holding what is left to a written list of seventeen with a reason each: the server half of `mp search`, which no slice migrated and which still dials IMAP or Graph itself; the startup line that warns about missing SMTP credentials, which prints before any socket exists; `mp config show`'s password column and OAuth token line, which are read locally on purpose because a redacted read is not allowed to probe your keyring; and the two `config.toml` setup wizards, which prompt, test the connection you just described and write the file in one interactive pass. The TUI is unchanged and still calls the library directly; it moves next.
- **The daemon now owns an account, its configuration, its drafts on disk and the files a viewer has open (#0122).** Phase 3a gave the daemon a spine that owned nothing; this is the phase where the ownership moves, and it is the first one with a change you can see without the `daemon` cargo feature. Two `mp` processes could always sync the same account at once: the per-account engine lock guarded the two queue drains only, so `mp sync` in one terminal and an open TUI in another both downloaded and ingested the same window into the same store. The lock now covers the ingest half too, and a second engine is refused before it opens a session, which costs no login and no server traffic. The refusal is a success rather than an error, the shape a refused outbox drain already had: `mp sync` prints `Sync skipped: another engine is syncing '<account>'; leaving the ingest to it` and exits 0, and the TUI says the same thing on its status line. The Microsoft Graph path still runs its own loop outside the engine and is not covered. Everything else is still behind the feature and still invisible to an installed `mp`, whose help output is byte-identical to the pre-daemon baseline in both builds. Inside it, an account runtime holds that lock for as long as the daemon lives, so the daemon is the account's engine rather than a visitor: it runs the same three-phase tick the TUI does, drains the outbox before the mutation queue at each end, joins a second sync to the one already running instead of starting a rival, and reports itself blocked rather than fighting an engine another process holds. Two read connections per account sit beside it, so a preview read is answered while a list load and a write are in flight. A finished tick is published as counts and a severity, never as a rendered sentence: `sync.completed` carries what was saved, skipped, pruned, rebound and deferred, and the wordings the CLI and the TUI print are derived from those counts by one shared function, so a GUI can render its own without reverse-engineering a string. The configuration moves in too. The daemon holds the live snapshot, and a reload validates the whole candidate before it stops anything, then stops removed accounts, restarts changed ones, starts added ones and announces itself last, so everything the reload did is already true when a client hears about it; an edit that does not parse leaves the previous configuration running and reports the file and the line. A first run with no `config.toml` can write one through the daemon, a password set through it is redacted in every log line, error and read-back, and removing an account releases its engine lock before the call returns. Two settings were quietly wrong and are fixed by the same work: an account with no `[accounts.smtp]` table at all loaded port 0 where an account with an empty table loaded 465, and the same omission asked for an unbounded body fetch instead of the 30-second default. The one-second poll the TUI runs over the drafts directory moves into the daemon and covers every account's drafts and the shared signatures directory, so an external edit becomes an event. An editor's save dance is one event about the final state rather than a storm: a temporary file renamed over the target, an original renamed away and recreated, and four saves in a row all settle once. A draft that will not parse stays a row a client can see, with the line the parser choked on, and it blocks approval instead of vanishing; the daemon never rewrites or restores a draft it could not read, which is also why a draft with no id is announced under its file name rather than having one written into a file an editor may be holding open. Finally, an attachment or an HTML rendition a client materialises is a handle, and a live handle pins the cached blob behind it, so the retention sweep cannot delete a file out from under a viewer that has it open; the sweep's two safety rules are untouched, `--force` still overrules the fat-finger guard and never a pin, and an unreleased handle is reclaimed by expiry rather than by deletion.
- **The daemon can now hold state, tell clients when it changes, and run work that outlives the call that asked for it (#0121).** Still nothing you run changes: this is all behind the same `daemon` cargo feature as #0120, `cargo install --path .` ships an unchanged `mp`, and `mp --help` is byte-identical to the pre-daemon baseline in both builds. What landed is the spine the ported clients will hang off. Methods no longer sit in a hand-written match in the server: each one declares whether it reads, changes state at once, runs long enough to be worth cancelling, or is work only the client's own process can do, and what its answer carries follows from that declaration rather than from what its author remembered. The daemon derives the capability list it advertises from that same table, so a method cannot be served without being advertised or advertised without being served. `state.bootstrap` hands a client the whole state it mirrors, plus the revision it was captured at, as one serialised operation: the connection is registered for events before the snapshot is taken, and the client drops every event at or below the revision it was given, which together mean a change made anywhere around a bootstrap arrives exactly once, never twice and never not at all. After that, changes arrive as events on the connection, and the shape of an event is chosen for what a client does with it: a small whole thing (an account becoming ready, a draft changing) is replaced outright, while a count is an instruction to re-read one query, which is what lets a hundred count changes for one mailbox collapse into one thing to do. Each connection has its own outbound queue holding at most 512 events or 4 MiB, so a client that stops reading costs the daemon a bounded amount of memory and delays nobody; when it overflows, the daemon throws away that connection's queued state events and asks it to bootstrap again rather than sending it a story with a hole in it. Long-running work answers immediately with an id and reports progress as events, any connection can read or cancel any operation because the daemon serves one person's mail and a sync started from a GUI should be watchable from the terminal beside it, a cancel is a fact by the time it answers rather than a promise, and closing a client's socket cancels only the work that connection declared client-scoped. A third read-only method, `mailbox.list`, answers one account's sidebar hierarchy with its counts, from the same hierarchy the TUI sidebar builds, so the two cannot disagree about which mailboxes an account has. None of it takes an account's engine lock, so a running TUI or `mp sync` still keeps working beside the daemon.
- **The daemon exists, speaks a pinned protocol, and answers two read-only questions over a socket (#0120).** Nothing you run changes: `cargo install --path .` still ships an `mp` that contains not a byte of this, because the whole of it sits behind a `daemon` cargo feature that stays off until the clients are ported. What landed is the transport the CLI, the TUI and a later GUI will all share. Two new crates carry it: `mp-protocol` owns the wire, and `mp-client` owns one connection and the handshake on it, neither of them depending on mp itself, so a GUI can link the client alone and cannot reach the engine by accident. The protocol is JSON-RPC 2.0, one message per line over a Unix socket, capped at 1 MiB per request and enforced on the buffer rather than on a completed line, so a client that streams past the cap without ever ending its frame is cut off instead of buffered. Every public shape has a committed JSON fixture that is parsed, re-serialised and compared byte for byte on every test run, which makes a change to a field name or an error number a deliberate act with a changelog entry rather than a diff nobody notices. `mp daemon run`, `start`, `status`, `stop` and `restart` manage the process. A start takes an advisory lock whose lifetime is the file descriptor's, so the kernel releases it however the process died, and it holds that lock across both the spawn and the wait for readiness, which is what makes two simultaneous starts produce one daemon rather than two racing for the same socket. Readiness is a real round trip over the socket, not the appearance of a file, because a socket inode exists before anything is accepting on it. The runtime directory is user-only and tightened rather than trusted on every start, the socket is created at mode 0600 with no window at a looser one, and a socket left behind by a crash is removed only after a connection attempt proves nobody is listening and the file is ours: anything else, a regular file at that path, another user's socket, a stray permission bit, is reported with its reason and left exactly where it is, because a daemon that deletes files it does not understand is a daemon that deletes your mail. A client that cannot work with this daemon is told why, in terms it can act on. A mismatched protocol range, a capability the daemon does not offer, and a data or config directory that differs from the daemon's each have their own error, and the directory one names all four paths so you can see which override to drop. `daemon.status` and `daemon.stop` deliberately answer before any of that negotiation, which is what lets `mp daemon status` describe, and `mp daemon restart` end, a daemon this build cannot otherwise talk to. Two domain methods are served, `account.list` and `message.list`, and both only read; neither takes an account's engine lock, so a running TUI or `mp sync` keeps working while the daemon answers listings beside it. A listed row carries both of a message's dates, the sort key every stack orders by and the `Date:` header a listing prints, because neither can be derived from the other and a client that had only the first would have to open a store to render a column. The response direction has a cap of its own, 16 MiB, read by both sides from one constant: the daemon refuses to write a reply above it and says so on the request's own id, and a client sizes its decoder by it, so an oversized answer is a named error on both ends rather than a truncated frame on one and a dropped connection on the other. `mp --help` is byte-identical to what it was before any of this, in both builds, which is the property the phase is really measured on.
- **The four decisions the daemon's protocol rests on now have numbers behind them (#0119).** Nothing in mp changes: this is a throwaway benchmark harness outside the build, four decision records, and a proof. The protocol the daemon will speak is expensive to change once clients read it, and three of its shapes are questions a preference cannot settle, so they were measured against the real store through the real read path before Phase 2 fixes anything. A JSON-RPC round trip over a Unix socket costs 0.024 ms more at p95 than the same read done as a direct library call, which is 0.5% of the 5 ms budget the plan set, so JSON-RPC stays and the cache-and-prefetch contingency that was hanging over the preview path is not needed. A 5000-row mailbox travels whole, kept current by row-level delta events, because per sync event whole and paged are both sub-millisecond and paging would move jump-to-date, the metadata filter and select-all to the server, which is a behaviour change inside a migration whose whole criterion is that behaviour does not change. A payload over 1 MiB travels as a temp-file handle rather than a frame, which takes a 10 MiB body 32% faster because the body never gets JSON-escaped, and one frame below that threshold. An account holds two read connections, where one puts a preview read behind whatever long read is already running (11.4 ms worst case) and four buys nothing over two. The ordering algorithm that will let two clients share one authoritative state was also built and raced: a mutation forced at each of the five points around a bootstrap reaches the client exactly once, either in the snapshot or as one event, with no boundary needing a fallback. And the dependency question the plan left open is answered in writing, from crates.io rather than memory: no new crate is adopted, `schemars` reopens at Phase 7 and nothing else reopens on a date, and verifying a connecting client's uid needs no crate at all, since tokio already implements it on both Linux and macOS.
- **The behaviour mp has today is written down, so the move to a daemon can be checked against it rather than remembered (#0118).** mp is being restructured around a background daemon that owns every store read, every network call and every durable operation, with the CLI and the TUI as clients of it and a native GUI after that. The acceptance criterion for all of it is that nothing you can observe changes, and that criterion needs something to compare against, so this release freezes the current surface as committed artifacts under `docs/baselines/pre-daemon/`: the whole `mp --help` surface (50 screens, captured by a script that reimplements the walk the help snapshot test performs, so the two cannot drift), `mp dump-keys --json` (92 bindings, verified byte-identical to the copy the website publishes), and an inventory of the 152 keys the TUI dispatches by hand inside its overlays, which `dump-keys` cannot see and which a port would otherwise drop silently. Beside them, `docs/parity-matrix.md` classifies all 131 capabilities of the CLI and the TUI into what the GUI must deliver, what is a machine-facing surface for scripts, what is an operator diagnostic, what is daemon administration, and what is a one-way migration, each with the source that implements it today and the coverage that tests it. A new test walks the TUI's own sources and asserts that the set of engine modules it imports still matches a checked-in list of 12, so the boundary the migration has to close cannot widen while it is being closed. One instrument was added to the product itself: the store read that builds the preview body is now timed separately from the paint (`[TIMING] tui_preview_query` in the log), because the daemon puts a round trip in the middle of exactly that read and the two costs have to be separable before it does. `examples/mkfixture.rs` builds a deterministic offline mail store so the benchmarks in `docs/baselines/pre-daemon/workloads.md` can be rerun by anyone, with the numbers this host could take recorded next to the five that need a terminal and a real account and are marked `NOT TAKEN` rather than approximated. Nothing here changes what mp does: no command, flag, key or output moved, the 18 golden frames and the help snapshot are untouched, and the whole of it is documentation plus one log line.
- **A sync that keeps downloading the same messages now says so (#0115).** For weeks, every pass on one account re-downloaded the same 16 messages, about 34 MB, from the same mailbox, and reported it as an ordinary sync: nothing errored, so nothing was logged above info, and a pass that downloads mail it already holds looks exactly like a pass catching up on a backlog. #0112 fixed the cause; this is the detector, so the next instance is loud instead of silent. A pass that saw the whole mailbox, downloaded everything it owed and wrote all of it has nothing left to fetch, so mp now remembers what such a pass downloaded, per mailbox, and counts how many consecutive passes downloaded exactly the same messages. At two in a row it warns in the log with the mailbox, the account, the count and the UID range, again at three, then every tenth pass, so a bug that persists for a week stays visible without a line per tick. The TUI status line carries `fetch not converging on <mailbox> (see the log)` and turns amber rather than showing a green "Synced", and `mp sync` prints a warning line per affected mailbox; its exit code is unchanged, because nothing failed, what is wrong is that the work repeats. Only a complete pass counts: one cut short by the fetch deadline (#0113), one whose listing came back short, one that failed to write a message, and a dry run all leave the count untouched rather than resetting it, since they are expected to leave work behind. A mailbox renumbered server-side clears the count with the rest of its cursor state, and so does the first pass that finds nothing new, which is the fetch converging. The detector watches IMAP accounts only: Exchange accounts synced over Graph run a separate loop that never reports it, since the account this came from is an IMAP one.
...
### Changed
- **`mp daemon` and `mp account` are listed in `mp --help`.** Both were hidden while the daemon was behind a cargo feature; only the debug `--daemon` flag stays hidden.
- **The pre-daemon help byte-identity gates are retired.** The help text may now evolve after the cutover; the sync and send slices check only that each subcommand still offers the pre-daemon flags, and the divergences are listed in `docs/baselines/pre-daemon/README.md`.
...
### Fixed
- **A send, sync or search that finished while the TUI was reconnecting no longer leaves the spinner running (#0121).** After every re-bootstrap the TUI asks `operation.status` about each operation it still awaits and lands the result line the lost `operation.finished` would have shown; an id the daemon no longer knows is dropped.
...
- **The account watcher belongs to its runtime, so a reload leaves one watcher triggering ticks.** The old watcher's IDLE round in flight still holds its connection for up to 300 s.
...
- **Help lines no longer repeat clap's own default, and `-A` no longer claims `(default: first in config)`.** That default was wrong on `mp calendar rebuild`, `mp contacts rebuild` and `mp cutover`.
- **The `mp dump-keys` header names the real keymap path.**
- **A bullet list no longer renders a size larger than the prose around it in Apple Mail (#0127).** A message whose body mixed paragraphs and a list came out of Apple Mail's reading pane with the bullets a quarter larger than everything else, while Outlook rendered the same bytes uniformly. The markup was not at fault: the `<ul>` sat inside the styled wrapper `<div>` exactly where it belonged, and WebKit laid the message out uniformly when given it as-is, in quirks mode, and inside a document carrying Mail's own `MUIWebDocument.css`. What Mail did was keep the wrapper's `font-size` for the `<ul>` and fall back to its own default for every `<p>`, and apply the wrapper's `line-height` nowhere. The mistake was ours: the whole message carried its font once, on that wrapper, and let it inherit, which leaves any client free to lose it for part of the content. Gmail and Outlook had already forced the `<head><style>` copy to be duplicated onto the wrapper for the same reason, and this is the same lesson one level deeper. The converter now stamps the style block's declarations onto each element it emits, so every `<p>`, `<ul>`, `<ol>`, `<li>`, `<blockquote>`, heading and table cell carries the font family, size and line height itself, and every link its colour. The `<style>` block and the wrapper stay for the clients that honour them. A `style` attribute the converter already wrote, the `text-align` on an aligned table cell, is extended rather than replaced and still wins, since it comes last in the block. `<pre>` and `<code>` take the size and line height only, so a code block keeps whichever monospace family the client picked. The quoted original of a reply or forward is passed through untouched: it carries the sender's own styling and is not ours to rewrite. The default `font_size` also moves from `12pt` to `16px`, the same computed size in a unit fewer clients mangle; an existing `config.toml` keeps whatever it says, so change it by hand to follow.
- **One slow mailbox no longer holds a whole sync tick (#0113).** A sync asked for every new message in a mailbox with a single command and then waited for it, however long it took: one tick spent 160 seconds downloading about 34 MB from a Sent mailbox, and while a tick runs, nothing else does, so every other mailbox, every other account and every archive, delete or send queued in the TUI waited behind it. The download is now cut into batches of 20 messages, and on a TUI sync each mailbox gets a time budget, 30 seconds by default, configurable per account as `[accounts.imap] body_fetch_deadline_secs` (`0` for the old unbounded behaviour, maximum 600). A mailbox that runs out of budget stops between batches, keeps everything it has downloaded, and picks the rest up on the next sync from where it stopped, so a large backlog arrives over several ticks instead of blocking one. The batches are taken newest first, so what a stopped pass has is the newest mail and what it owes is the backlog. A pass that stopped early is treated exactly like any other incomplete pass: it deletes nothing, since a message it did not download may be the copy that would justify a deletion elsewhere, and it does not advance the point a server-side flag delta resumes from, so no read or answered marker made in another client is skipped. It says so too, once in the log per mailbox and on the status line as `N mailbox(es) stopped at the fetch deadline (resuming next sync)`. `mp sync` is never cut short whatever the config says, since the sync you run by hand is the one that has to converge; nothing about a batch already asked for is abandoned mid-flight, so a connection is never left holding an unread answer.
