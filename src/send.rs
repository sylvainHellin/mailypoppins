...
mod tests {
    ...
    fn split_addresses_quoted_comma_in_display_name() {
        let r = split_addresses("\"Hellin, Sylvain\" <sylvain@hines.com>, bob@x.com");
        assert_eq!(r, vec!["\"Hellin, Sylvain\" <sylvain@hines.com>", "bob@x.com"]);
    }
    ...
    fn split_addresses_lettre_parses_quoted_name() {
        ...
        let addr = "\"Hellin, Sylvain\" <sylvain@hines.com>";
        let mbox: Mailbox = addr.parse().expect("lettre should parse quoted display name");
        assert_eq!(mbox.email.to_string(), "sylvain@hines.com");
    }
